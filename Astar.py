...
# This function is to parent node to smallest f value node and update the openSet
def update_set(current: Cell, neighbor: Cell, set: list):
    for n in set:
        if n.x == neighbor.x and n.y == neighbor.y:
            if n.parent.f > current.f:
                n.parent = current
    return set
...
# reconstruct path
def reconstruct_path(current_cell: Cell):
    print("find path")
    path = []
...
# Find the heuristic between the current cell to goal using Manhattan Distance
    ...
    while not (not openSet):
       ...
       # openSet.remove(current)
        ...
        for neighbor in neighbor_list:
            ...
            # Check whether the neighbor already exist in the openSet
            if not cell_in_set(openSet, neighbor):
                openSet.append(neighbor)
            else:
                openSet = update_set(current, neighbor, openSet)  #update openSet
